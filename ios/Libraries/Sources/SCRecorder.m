...
- (void)record {
    void (^block)() = ^{
        NSLog(@"Recording started! movieOutput = %p", _movieOutput);
        _isRecording = YES;
        if (_movieOutput != nil && _session != nil) {
            NSLog(@"Starting movie recording...");
    ...
    if ([SCRecorder isSessionQueue]) {
        NSLog(@"Record: Calling in session queue");
        block();
    } else {
        NSLog(@"Record: Dispatching in the queue");
